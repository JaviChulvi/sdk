...
This repository contains generated SDKs for Ultralytics products. `openapi.config.json` defines the consumer configuration; `openapi.json` and `openapi.sha256` pin the contract. All of `sdk/python/` is generated, committed, and drift-checked; never edit it manually. Maintain CLI behavior in root `cli.py` and shared credentials in `auth.py`. The generator copies these sources into the package, appends only multipart binary-field metadata and the CLI launcher, and registers `ul`. The CLI discovers operations, types, and help from SDK signatures and docstrings. After changing sources, configuration, or contract, regenerate with an `ultralytics/openapi` main checkout at `.generator/` and copy `.generator/generated/python/` to `sdk/python/`. `tests/` owns consumer checks; `format.yml` applies Ultralytics Actions, `ci.yml` owns regeneration and package validation, and `publish.yml` owns releases and PyPI publishing.
